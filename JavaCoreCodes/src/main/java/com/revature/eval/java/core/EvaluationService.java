...
public class EvaluationService {
	...
	public String cleanPhoneNumber(String string) {
		...
		for(int i=0; i<string.length(); i++){
			if( "0123456789".contains(""+(string.charAt(i)))){
				clnd += string.charAt(i);
			}
		}
		if(clnd.length()==11 && clnd.charAt(0)==1 && (clnd.charAt(1) > 2 && clnd.charAt(4) > 2)){
			clnd = clnd.substring(1);
		}
		if (clnd.length()>10)
			try{
	...
	public Map<String, Integer> wordCount(String string) {
		...
		for(int i=0; i<string.length(); i++){
			if(string.charAt(i)==' '){
				str += string.substring(string.charAt(i+1), string.charAt(i));
			}
		}
